...
const renderCart = async () => {
    const url = `http://localhost:8080/api/cart/${cartId}/productos`;
...
const finalizarCompra = (data) => {
    const url = `http://localhost:8080/api/cart/${cartId}/finalizar-compra`;
    fetch(url, {
        method: "POST",
...
const deleteProduct = (cartId, productId) => {
    const url = `http://localhost:8080/api/cart/${cartId}/productos/${productId}`;
    fetch(url, {
        method: "DELETE",
        headers: {
            "Content-Type": "application/json"
